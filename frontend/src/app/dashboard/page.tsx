...
export default function DashboardPage() {
  ...
  const Tip = ({ active, payload, label }: { active?: boolean; payload?: { value: number }[]; label?: string }) => {
    ...
    return <div className="matrix-panel px-3 py-2 text-xs font-mono"><p className="text-matrix-subtle mb-1">{label}</p><p className="text-matrix-green">{formatCurrency(payload[0]?.value || 0)}</p></div>;
  };
